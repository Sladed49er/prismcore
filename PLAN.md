...
**Accounting is fully built — all 18 sub-modules live.** `/m/accounting` is a
**hub**: Chart of Accounts, Journal Entries (real double-entry — won't post
unless balanced), Invoices, Vendors, Bills, Trust Accounting, Employees,
Payroll, Bank Reconciliation, Budgets (by GL account), Fixed Assets (computed
depreciation + book value), Estimates, Checks & Positive Pay, Fiscal Periods
(open/closed locking), Surplus Lines Tax (per-state, tax = premium × rate +
fees), Quarterly Taxes, **1099/W-2 Reporting** (computed from payroll + AP),
...
**Policies & Servicing is fully built — all 9 sub-modules live.** `/m/policies`
is a **hub** (Accounting pattern); the policy register moved to
`/m/policies/register`. Sub-modules: Policy Register, Coverages
(limits/deductibles/allocated premium), Endorsements (mid-term changes,
premium delta), Cancellations (flat/pro-rata/short-rate, return premium,
reinstatement), Premium Schedule (installment billing with mark-paid),
**Insured Schedules** (vehicles/drivers/locations/equipment), **Premium
Audits** (estimated vs. audited premium, computed adjustment), **Service
...
**Claims is fully built — all 7 sub-modules live.** `/m/claims` is a **hub**
(Accounting pattern); the claims register moved to `/m/claims/register`.
Sub-modules: Claims Register, Claim Diary (chronological notes), Reserves
(reserve-change ledger by indemnity/expense/legal/medical), Claim Payments
(loss & expense, pending/issued/cleared/voided), Recovery & Subrogation,
**Claim Parties** (claimants/witnesses/adjusters/attorneys/experts),
...
Claims (7). Next pillars: Commissions and Renewals.
