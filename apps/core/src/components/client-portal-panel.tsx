...
import {
  ...
  revokeInvite,
  deleteInvite,
} from "@/app/(shell)/m/client_portal/actions";
...
const STATUS_COLOR: Record<PortalInvitationDTO["status"], string> = {
...
export function ClientPortalPanel({
  ...
  function submit(): void {
    ...
    startTransition(async () => {
      await inviteClient({ clientId, email });
      setClientId("");
      setEmail("");
      setShowForm(false);
    });
    ...
    <div className="mt-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">
          {activeCount} client{activeCount === 1 ? "" : "s"} with portal access
        </p>
        {!showForm ? (
          <button
            type="button"
      ...
      {showForm ? (
        <div className="mt-4 rounded-xl border border-gray-200 bg-white p-5">
          ...
          <p className="mt-3 text-xs text-gray-500">
            ...
            client so they can view their policies. No password required.
          </p>
      ...
      <div className="mt-5 overflow-hidden rounded-xl border border-gray-200 bg-white">
        {invitations.length === 0 ? (
          <p className="px-5 py-8 text-center text-sm text-gray-500">
            No clients invited to the portal yet.
          </p>
        ) : (
