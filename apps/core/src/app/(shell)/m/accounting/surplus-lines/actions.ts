...
export async function editSurplusLines(input: {
  id: string;
  policyReference: string;
  state: string;
  premiumDollars: string;
  ...
  if (!input.id || !input.policyReference.trim()) return;
  const tenant = await getCurrentTenant();
  const dollars = (v: string): number =>
    Math.round((Number.parseFloat(v) || 0) * 100);
  await updateSurplusLines({
    tenantId: tenant.id,
    id: input.id,
    policyReference: input.policyReference.trim(),
    state: input.state.trim(),
...
export async function removeSurplusLines(id: string): Promise<void> {
  if (!id) return;
  const tenant = await getCurrentTenant();
  await deleteSurplusLines(tenant.id, id);
  revalidatePath("/m/accounting/surplus-lines");
}
